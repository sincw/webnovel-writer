...
> 本步骤执行 `anti-ai-guide.md` 的 **Patch B：二次自审降AI**。
> 完整规则见 `references/anti-ai-guide.md`，以下为执行摘要。

### 执行流程

1. **切换身份**：以”起点写了八年的老油条网文作者”视角审视全文
2. **识别疑似AI段落**：节奏太顺 / 句式太匀 / 情绪侧写重复 / 逻辑链像步骤 / 过渡太丝滑 / 收尾太干净（命中 2 项即为疑似）
3. **强制破坏**（每个疑似段落至少执行 2 项）：
   - 打乱句子顺序（先结果再过程）
   - 过渡词替换为糙口语（话说、哎、结果呢、搁这儿、这不、谁想到）
   - 删掉”干净完整”的小结尾
   - 加突兀的生理/环境碎念
4. **全文终检**：是否还有”干净文本味”？有则继续弄脏弄乱

### AI高频词黑名单

| 类型 | 禁用词 |
|------|--------|
| 总结词 | 综合 / 总之 / 由此可见 / 总而言之 / 可以说 |
| 列举结构 | 首先 / 其次 / 最后 / 第一 / 第二 / 第三 |
| 学术词 | 而言 / 某种程度上 / 本质上 / 值得注意的是 |
| 元叙述 | 不禁 / 心中一震 / 暗暗想到 / 心中暗道 |
| 华丽空词 | 宛如 / 犹如 / 仿佛…一般 / 不由得（偶尔可用，堆砌则禁） |

绝对禁止使用以下词汇及其变体（包括但不限于）：心中一震、心头一颤、不由得、宛如、犹如、仿佛…一般、总之、总而言之、由此可见、首先、其次、值得注意的是、某种程度上、本质上、蓬勃发展、日新月异、赋能、探索、整合、优化、深刻、深远。

想用这些词时，强制换成最直白、最口语的表达，或者直接删除句子。心理描写必须全靠动作、感官、吐槽、自言自语侧写，禁止任何情绪直述标签。
